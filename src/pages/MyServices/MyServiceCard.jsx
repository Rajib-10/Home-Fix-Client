  ...
  const handleDelete=id=>{
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        axios.delete(`http://localhost:5000/add-services/${id}`)
    .then(res=>{
    ...
    return ( 
      ...
      <motion.div whileHover={{ scale: 1.05, transition: { duration: 0.3 } }} className="flex flex-col  p-6 space-y-6 overflow-hidden rounded-lg shadow-md bg-base-100">
        ...
        <div className="flex justify-between items-center">
            ...
            <button className="btn bg-orange-400 hover:bg-orange-500 text-white">Update Service</button>
            </Link>
            <button onClick={()=>handleDelete(myService._id)} className="btn bg-red-400 hover:bg-red-500 text-white">Delete Service</button>
