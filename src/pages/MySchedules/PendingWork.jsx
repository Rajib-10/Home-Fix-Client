...
const PendingWork = () => {
  ...
  useEffect(() => {
    axios
      .get("https://home-fix-server.vercel.app/booking-services", {
        withCredentials: true,
      })
      .then((res) => {
        const filterPendingWorks = res.data.filter(
          (item) => item.providerEmail == user?.email
        );
  ...
  const handleStatusChange = (event, bookingId) => {
    ...
    axios
      .patch(
        `https://home-fix-server.vercel.app/booking-services/${bookingId}`,
        { status: newStatus }
      )
      .then((res) => {
        if (res.data.modifiedCount > 0) {
          const remaining = pendingWorks.filter(
          ...
          {pendingWorks.length < 1 ? (
            <div className="flex justify-center items-center h-[50vh]">
              <h1 className="text-3xl text-red-500">
                You do not have any pending works..
              </h1>
            </div>
          ) : (
                    ...
                    <tr key={booking._id}>
                      ...
                      <td>
                        <select
                          ...
                          onChange={(event) =>
                            handleStatusChange(event, booking._id)
                          }
                        >
