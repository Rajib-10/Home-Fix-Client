...
const SingleService = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://home-fix-server.vercel.app/add-services/${id}`, {
  ...
  useEffect(() => {
    axios
      .get(`https://home-fix-server.vercel.app/add-services`, {
        withCredentials: true,
      })
      .then((res) => {
        const filterRestServices = res.data.filter(
          (item) => item.email == email && item._id !== id
        );
  ...
  const handlePurchase = (e) => {
    ...
    axios
      .post("https://home-fix-server.vercel.app/booking-services", booking)
      .then((res) => {
    ...
    <div>
          ...
          <div>
            ...
            {user?.email === email && (
              <span className="text-red-700 mx-1">
                you are service provider, you can not book service
              </span>
            )}
      ...
      <div>
        {restServices.length > 0 && (
          <div>
            <h1 className="text-3xl text-orange-400 p-8">
              More service by same provider
            </h1>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 my-6 w-[90%] mx-auto">
              {restServices?.map((restService) => (
                <RestServiceCard key={restService} restService={restService} />
              ))}
            </div>
          </div>
        )}
